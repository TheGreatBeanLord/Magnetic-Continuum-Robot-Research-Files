...
num_segments = 2; 
EI = [1.05e-4, 1.05e-4]; % Bending stiffness (Nm^2)
L = [0.04, 0.04]; % Lengths of each bending segment (m)

% Magnetic parameters
mu0 = 4 * pi * 1e-7; % Permeability of free space (T*m/A)
M = 5.44e-3; % Dipole moment magnitude (Am^2) for small internal magnets
...
initial_theta =[0.01, 0.01]; % Example initial guess
...
fprintf('Disclaimer: Energy Values only have meaning relative to one another; this is not the actual stored energy in the system, but the relative energy conforming to elastic and electromagnetic conventions');
...
angle_accumulated = start_angle;
for i = 1:num_segments
    R = L(i) / initial_theta(i);
    x = x0 + R * (sin(angle_accumulated + initial_theta(i) * t) - sin(angle_accumulated));
    z = z0 + R * (cos(angle_accumulated) - cos(angle_accumulated + initial_theta(i) * t));
    plot(x, z, 'g', 'LineWidth', 1.5); % Dashed black for original configuration
    x0 = x(end); z0 = z(end);
    angle_accumulated = angle_accumulated + initial_theta(i);
end
...
xlabel('X Position (m)');
ylabel('Z Position (m)');
legend('Stable Configuration Segment 1', 'Stable Configuration Segment 2','Unstable Start Configuration');
hold off;
